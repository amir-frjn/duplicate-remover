...
use duplicate_remover::fc_generator::models::{HashTypes, SearchAlgorithm, SearchOptions};
fn main() {
...
use duplicate_remover::fc_processor::operations;
fn show_duplicates() {
    if let Err(e) = operations::show_duplicates("files.db") {
        eprintln!("An error occurred: {e}");
    };
}
use duplicate_remover::fc_generator::hasher;
